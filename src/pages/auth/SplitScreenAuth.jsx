...
const roles = [
  ...
  { key: 'customer', title: 'Customer', accent: '#406F60', subtitle: 'Track purchases, verify sustainability, and follow provenance.', cta: 'Enter Customer Portal' },
];

const fieldClass = 'w-full rounded-xl border-2 border-[#d4c4af] bg-white p-3 outline-none ring-primary/30 focus:border-primary focus:ring';
    ...
    <div className="relative min-h-screen overflow-hidden bg-cream px-4 py-10 sm:px-6 lg:px-8">
      ...
      <div className="relative mx-auto grid w-full max-w-6xl gap-6 lg:grid-cols-[0.95fr_1.05fr]">
        <motion.aside
          ...
          className="relative overflow-hidden rounded-[2rem] border-2 border-[#184637] bg-gradient-to-br from-[#123e31] via-[#1F5C4A] to-[#255849] p-6 text-white shadow-[0_30px_70px_rgba(18,62,49,0.45)] sm:p-8"
        >
          <div className="absolute -right-20 -top-20 h-56 w-56 rounded-full bg-white/10 blur-3xl" />
          <div className="relative">
            <p className="text-xs uppercase tracking-[0.24em] text-[#d8efe8]">Live Role Preview</p>
            <h2 className="mt-3 text-3xl font-bold">{role.title} Dashboard Access</h2>
            <p className="mt-3 text-sm leading-6 text-[#dceae5]">{role.subtitle}</p>
            <div className="mt-8 space-y-3">
              {[
                ...
                >
                  <p className="text-sm font-semibold text-[#f0fffa]">{title}</p>
                  <p className="mt-1 text-xs leading-5 text-[#d7ebe5]">{text}</p>
                </motion.div>
              ))}
            </div>
          </div>
        ...
        <motion.section
          ...
          className="glass-card border-2 border-[#d8c7b1] p-6 sm:p-8"
        >
          <div className="mb-6">
            <p className="text-xs uppercase tracking-[0.3em] text-[#255849]/80">Nasuo Hive Access</p>
            <h1 className="mt-2 text-3xl font-bold text-[#173c31] sm:text-4xl">Choose Role and Sign In</h1>
            <p className="mt-3 text-sm text-[#35584b] sm:text-base">Single gateway for manufacturers, retailers, and customers.</p>
          </div>
            ...
            {roles.map((item) => {
              ...
              return (
                <motion.button
                  ...
                  style={{ borderColor: active ? item.accent : '#d7c6b1' }}
                >
                  <div className="flex items-center justify-between">
                    <p className="text-sm font-semibold" style={{ color: active ? item.accent : '#35584b' }}>{item.title}</p>
                    <span className="inline-flex h-2.5 w-2.5 rounded-full" style={{ backgroundColor: active ? item.accent : '#aac1b9' }} />
                  </div>
          ...
          <div className="mt-6 rounded-2xl border-2 border-[#dfcfbc] bg-[#fff9f2] p-4">
            <div className="mb-4 flex rounded-full bg-[#efe2d2] p-1">
              <button type="button" onClick={() => setMode('login')} className={`w-1/2 rounded-full py-2 text-sm font-semibold transition ${mode === 'login' ? 'bg-primary text-white' : 'text-[#35584b]'}`}>Login</button>
              <button type="button" onClick={() => setMode('register')} className={`w-1/2 rounded-full py-2 text-sm font-semibold transition ${mode === 'register' ? 'bg-primary text-white' : 'text-[#35584b]'}`}>Register</button>
            </div>
            ...
            <form onSubmit={submit} className="space-y-3">
              ...
              <div className="relative">
                ...
                <button type="button" onClick={() => setShowPassword((prev) => !prev)} className="absolute right-2 top-1/2 -translate-y-1/2 rounded-full border border-[#d8c7b1] bg-white p-1.5 text-[#35584b]" aria-label={showPassword ? 'Hide password' : 'Show password'}>
                  {showPassword ? (
              ...
              <motion.button whileHover={{ y: -1 }} whileTap={{ scale: 0.99 }} type="submit" className="w-full rounded-full px-5 py-3 text-sm font-semibold text-white" style={{ background: `linear-gradient(180deg, ${role.accent}, #123e31)` }}>
                {mode === 'login' ? role.cta : `Create ${role.title} Account`}
              </motion.button>

              <div className="relative my-4">
                <div className="absolute inset-0 flex items-center"><div className="w-full border-t border-[#d9cab8]" /></div>
                <div className="relative flex justify-center text-xs"><span className="bg-[#fff9f2] px-2 text-[#546760]">Or continue with</span></div>
              </div>

              <div className="grid grid-cols-2 gap-3">
                <button type="button" onClick={() => handleSocialLogin('Google')} className="flex items-center justify-center gap-2 rounded-full border-2 border-[#d7c6b1] bg-white py-2.5 text-sm font-semibold text-[#35584b] transition hover:bg-[#f8efe4]">
                ...
                <button type="button" onClick={() => handleSocialLogin('Microsoft')} className="flex items-center justify-center gap-2 rounded-full border-2 border-[#d7c6b1] bg-white py-2.5 text-sm font-semibold text-[#35584b] transition hover:bg-[#f8efe4]">
